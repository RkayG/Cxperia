-- Debug script for experience slug L7i6TZUx
...
SELECT 
    ...
    created_at
FROM experiences 
WHERE public_slug LIKE '%L7i6TZUx%' 
   OR public_slug LIKE '%L7i6%'
...
WHERE e.public_slug = 'L7i6TZUx';

-- 6. Check if the experience exists but is not published
SELECT 
    'Unpublished Experience Check:' as info,
    id,
    public_slug,
    name,
    is_published,
    created_at
FROM experiences 
WHERE public_slug = 'L7i6TZUx' AND is_published = false;
