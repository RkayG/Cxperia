...
const ExperienceOverviewSection: React.FC<ExperienceOverviewProps> = ({ data, onUpdate, isLoading: externalLoading }) => {
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    (async () => {
      try {
        ...
        if (url) {
          ...
          try {
            ...
            if (resp && (resp as any).success && (resp as any).data && ((resp as any).data as any).logo_url) {
              ...
              showToast.success('Logo de marque mis à jour');
            } else if (resp && (resp as any).success) {
              showToast.success('Logo de marque mis à jour');
            } else {
              showToast.error('Échec de la sauvegarde du logo de marque');
            }
          } catch (e) {
        ...
        setEditData(prev => ({ ...prev, logoFile: file }));
        onUpdate({ logoFile: file });
        showToast.error('Échec du téléchargement du logo');
      } finally {
  ...
  return (
    <div className="bg-[#ede8f3] rounded-xl   mb-8 md:mb-12 border border-purple-800 p-4">
      ...
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-900">Aperçu de l'expérience</h2>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <div className="mb-2">
                <span className="text-gray-700 font-medium text-xs uppercase tracking-wide">Expérience:</span>
                <p className={`font-medium text-sm ${data.experienceName ? 'text-gray-900' : 'text-gray-400'}`}>
                  {data.experienceName ? data.experienceName : '...'}
                </p>
              </div>
              <div className="mb-2">
                <span className="text-gray-700 font-medium text-xs uppercase tracking-wide">Slogan:</span>
                <p
            ...
            <div>
              <div className="mb-2">
                <span className="text-gray-700 font-medium text-xs uppercase tracking-wide">Catégorie:</span>
                <p className="font-medium text-sm text-gray-900">{data.category || ''}</p>
              </div>
              <div>
                <span className="text-gray-700 font-medium text-xs uppercase tracking-wide">Magasin:</span>
                <p> 
                  <a 
                    href={data.storeLink || ''} 
                    target="_blank" 
          ...
          <div className="bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 p-3 relative">
            ...
            {isUploading && (
              <div className="absolute inset-0 bg-white/80 rounded-lg flex flex-col items-center justify-center z-10">
                ...
                <p className="text-xs font-medium text-gray-700">Téléchargement...</p>
