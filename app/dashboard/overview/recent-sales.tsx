  ...
  if (recentFeedbacks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        ...
        <p className="text-sm text-muted-foreground">Aucun feedback reçu encore</p>
        <p className="text-xs text-muted-foreground mt-1">
          Les feedbacks apparaîtront ici lorsque les clients soumettront des avis
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      {recentFeedbacks.map((feedback, index) => {
        ...
        const timeAgo = date ? new Date(date).toLocaleDateString() : 'Récemment';
        
        // Get rating or feedback type
        const rating = feedback.rating || feedback.overall_rating || 0;
        const ratingDisplay = rating > 0 ? `${rating}★` : 'Nouveau';
