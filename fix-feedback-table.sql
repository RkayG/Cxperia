-- Fix customer_feedbacks table
-- First, drop the table if it exists with issues
DROP TABLE IF EXISTS customer_feedbacks CASCADE;

-- Create customer_feedbacks table with proper UUID generation
...
CREATE INDEX idx_customer_feedbacks_experience_id ON customer_feedbacks(experience_id);
CREATE INDEX idx_customer_feedbacks_brand_id ON customer_feedbacks(brand_id);
CREATE INDEX idx_customer_feedbacks_created_at ON customer_feedbacks(created_at DESC);
CREATE INDEX idx_customer_feedbacks_overall_rating ON customer_feedbacks(overall_rating);
...
-- Policy: Brands can view their own feedbacks (for dashboard)
...
SELECT 'customer_feedbacks table created successfully' as status;
